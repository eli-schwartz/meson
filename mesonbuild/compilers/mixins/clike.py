...
class CLikeCompiler(Compiler):
    ...
    def __init__(self, exe_wrapper  = None):
    ...
    @functools.lru_cache()
    def _get_library_dirs(self, env ,
                          elf_class  = None)  :
    ...
    @functools.lru_cache()
    def _get_program_dirs(self, env )  :
        '''
    ...
    def _sanity_check_impl(self, work_dir , environment ,
                           sname , code )  :
        mlog.debug('Sanity testing ' + self.get_display_language() + ' compiler:', ' '.join(self.exelist))
        mlog.debug('Is cross compiler: {!s}.'.format((self.is_cross)))
        ...
        try:
            ...
            raise mesonlib.EnvironmentException('Could not invoke sanity test executable: {!s}.'.format((e)))
        pe.wait()
        if pe.returncode != 0:
    ...
    def has_header(self, hname , prefix , env , *,
        ...
        #ifdef __has_include
         #if !__has_include("{}")
          #error "Header '{}' could not be found"
         #endif
        #else
         #include <{}>
        #endif'''.format((prefix), (hname), (hname), (hname))
        return self.compiles(code, env, extra_args=extra_args,
    ...
    def has_header_symbol(self, hname , symbol , prefix ,
        ...
        int main(void) {{
            ...
            #endif
            return 0;
        }}'''.format((prefix), (hname), (symbol), (symbol))
    ...
    def _compile_int(self, expression , prefix , env ,
                     extra_args    ,
                     dependencies )  :
        t = '''#include <stdio.h>
        {}
        int main(void) {{ static int a[1-2*!({})]; a[0]=0; return 0; }}'''.format((prefix), (expression))
        return self.compiles(t, env, extra_args=extra_args,
                             dependencies=dependencies)[0]

    def cross_compute_int(self, expression , low , high ,
                          guess , prefix , env ,
                          extra_args     = None,
                          dependencies  = None)  :
    ...
    def cross_sizeof(self, typename , prefix , env , *,
                     extra_args     = None,
                     dependencies  = None)  :
        if extra_args is None:
            extra_args = []
    ...
    def cross_alignment(self, typename , prefix , env , *,
                        extra_args  = None,
                        dependencies  = None)  :
        if extra_args is None:
            extra_args = []
    ...
    def get_define(self, dname , prefix , env ,
        ...
        # define {}
        #endif
        {}\n{}'''.format((prefix), (dname), (dname), (delim), (dname))
    ...
    def get_return_value(self, fname , rtype , prefix ,
                         env , extra_args ,
                         dependencies )   :
        ...
        int main(void) {{
            printf ("{}", {} {}());
            return 0;
        }}'''.format((prefix), (fmt), (cast), (fname))
        res = self.run(code, env, extra_args=extra_args, dependencies=dependencies)
        if not res.compiled:
            raise mesonlib.EnvironmentException('Could not get return value of {}()'.format((fname)))
        if rtype == 'string':
            return res.stdout
        elif rtype == 'int':
            try:
                ...
                raise mesonlib.EnvironmentException('Return value of {}() is not an int'.format((fname)))
    ...
    def has_members(self, typename , membernames ,
        ...
        void bar(void) {{
            {} foo;
            {}
        }}'''.format((prefix), (typename), (members))
        return self.compiles(t, env, extra_args=extra_args,
                             dependencies=dependencies)
    ...
    def symbols_have_underscore_prefix(self, env )  :
        ...
        with self._build_wrapper(code, env, extra_args=args, mode='compile', want_output=True, temp_dir=env.scratch_dir) as p:
            if p.returncode != 0:
                raise RuntimeError('BUG: Unable to compile {!r} check: {}'.format((n), (p.stdout)))
            if not os.path.isfile(p.output_name):
                raise RuntimeError('BUG: Can\'t find compiled test code for {!r} check'.format((n)))
    ...
    @staticmethod
    def _get_file_from_list(env , paths )  :
        '''
        ...
        for p in paths:
            ...
            if archs and env.machines.host.cpu_family in archs:
                ...
                mlog.debug('Rejected {}, supports {} but need {}'.format((p), (archs), (env.machines.host.cpu_family)))
        return None
    ...
    def _find_library_real(self, libname , env , extra_dirs , code , libtype )  :
    ...
    def _find_library_impl(self, libname , env , extra_dirs ,
                           code , libtype )  :
    ...
    def _find_framework_real(self, name , env , extra_dirs , allow_system )  :
        code = 'int main(void) { return 0; }'
        link_args = []
        for d in extra_dirs:
            link_args += ['-F' + d]
    ...
    def _find_framework_impl(self, name , env , extra_dirs ,
                             allow_system )  :
        if isinstance(extra_dirs, str):
            extra_dirs = [extra_dirs]
    ...
    def find_framework(self, name , env , extra_dirs ,
                       allow_system  = True)  :
        '''
    ...
    def has_arguments(self, args , env , code ,
                      mode )   :
        return self.compiles(code, env, extra_args=args, mode=mode)

    def _has_multi_arguments(self, args , env , code )   :
        ...
        for arg in args:
            ...
            if arg.startswith('-Wl,'):
                mlog.warning('{} looks like a linker argument, '
                             ...
                             'other similar method can be used instead.'.format((arg)))
            new_args.append(arg)
    ...
    def _has_multi_link_arguments(self, args , env , code )   :
    ...
    @staticmethod
    def _concatenate_string_literals(s )  :
